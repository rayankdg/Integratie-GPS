...
class FcmReceiver:
    ...
    def get_android_id(self):
        ...
        # Na registratie zijn de credentials gezet -> geef het echte android_id terug.
    ...
    async def _register_for_fcm(self):
        ...
        while fcm_token is None:
            try:
                ...
                # stop() kan zelf crashen als er nog niets gestart is -> afschermen.
                try:
                    await self.pc.stop()
                except Exception:
                    pass
                ...
                if attempts >= 12:
                    raise RuntimeError(
                        f"FCM-registratie blijft falen ({e}). Meestal tijdelijke "
                        "rate-limiting van Google -- wacht enkele minuten en probeer opnieuw."
                    )
                print(f"[FCMReceiver] FCM-registratie mislukt ({e}). Opnieuw proberen...")
